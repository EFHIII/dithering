...
export function energyWithSwaps({
  imgData,
  palette,
  pattern,
  patternSize,
  ...
  do {
    ...
    pixelLoop:
      for(let index of pixelOrder) {
        ...
        if(Math.random() > 0.5) {
          const firstTemp = pixelsToChange[0][6];

          for(let i = 0; i < pixelsToChange.length; i++) {
            const p = pixelsToChange[i];
            const oldCandidate = ditheredIndexes[p[2]];
            const newCandidate = (i === pixelsToChange.length - 1) ? firstTemp : pixelsToChange[i + 1][6];
            const oldLRGB = paletteLRGB[oldCandidate];
            const newLRGB = paletteLRGB[newCandidate];
            pixelsToChange[i][3] = newLRGB[0] - oldLRGB[0];
            pixelsToChange[i][4] = newLRGB[1] - oldLRGB[1];
            pixelsToChange[i][5] = newLRGB[2] - oldLRGB[2];
            pixelsToChange[i][6] = newCandidate;
          }
        }
